...
- **Phase 5**: iOS launch — **Swift native + TestFlight** (decided 2026-05-07; in active development since 2026-05-08). MVP streams W1a, W1.5, W2, W3, **W4, W4.5, W5, W5.1, W6, W7** are merged to `main`, plus 3 PRs against tile / SW pipelines (#21 + #22 intersection-overshoot fix, #23 SW cache bump). See "Phase 5 progress" table above for stream status. Next stream: **W7.5** ("Park Until X" filter) or **W6.1** (deep-link tap presentation flake — `@tech-lead` to spec, ~half session). After: W7.5 → W8 (TF1, blocked on Apple Dev approval) → W8.5 (Drive Mode, vision-focused port per `docs/drive-mode-scope-spec.md`) → W9 (TF2). v1.0 App Store launch ships with Drive Mode included. Post-MVP follow-on phases: threat tracker UI, zone chat, Smart Move recommendations, address search, snow emergency / NYC 311 API, paywall + StoreKit (see `docs/business-model.md`).
