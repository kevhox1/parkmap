...
| **W4** — Block detail sheet on tap | 📋 **spec ready, awaiting `@ios-engineer` dispatch** | Full spec at `docs/w4-block-detail-spec.md` (locked 2026-05-10). All decisions made: `.sheet(item:)` with `.medium`/`.large` detents, invisible 20pt-wide polyline tap overlay (MapAnnotation midpoint fallback), severity color band + block header + safety label + rule list + disabled "Park here →" stub for W5 entry. Selected-block highlight via `@State` + ternary on `lineWidth`. Accessibility labels (W3 QA #8) absorbed here. R1 stress test on real device required before merge. Branch: `ios/w4-block-detail`. |
...
- W3 QA minor findings (#6, #7) — informational/deferrable cosmetic items. See `docs/qa/w3-pass-1-2026-05-10.md`. (Note: W3 QA #1, #2, #5 were already in-fact merged with PR #15 — the QA report was written against an earlier branch snapshot before the engineer fixed them; verified 2026-05-10 on the new dev machine. W3 QA #8 [accessibility labels on `MapPolyline`] is correctly deferred and is absorbed into W4 §3.5.)
