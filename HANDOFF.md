...
| **W5.1** — W5 polish (recenter button + 2 QA fix-pass items) | 📋 queued | (1) Recenter / find-me button on the map — Kevin reported getting "lost" when zooming out during W5 smoke (pin floating in NY Harbor, no easy way to return). Likely `MKUserTrackingButton` or custom overlay; support recenter-on-user AND recenter-on-pin if pin exists. (2) Move `pinDropped.send(car)` inside the do-catch block in `ParkPinService.save()` per QA Finding #1 — must land before W7.5 begins because W7.5 will trust the hook contract. (3) Refresh `alternativeCandidates` after a "Wrong street?" tap so the selected block is no longer shown as an alternative (QA Finding #2). Small PR, ~half engineer session. |
| **W6** — Local notifications + permission flow | ⏳ | `UNUserNotificationCenter`. Rationale sheet on first pin drop (per spec §3.4). `ParkPinService.firstPinDropped` Combine hook from W5 is the attachment point. |
...
- W5 QA pass-1 findings (`pinDropped` hook ordering + alternatives-list stale state) — folded into W5.1 scope; see `docs/qa/w5-pass-1-2026-05-12.md`.
- W5 unit tests for `ParkPinService` round-trip + `findCandidateSegments` haversine — QA Finding #3, optional. No regression risk; consider during W5.1 if engineer has bandwidth.
- **Polyline intersection geometry artifacts.** Kevin flagged in W5 smoke that block-face polylines occasionally cross into intersection geometries. Pre-existing — same tile data feeds both apps; PWA exhibits the same artifact. Fix lives in the tile build pipeline (`scripts/build-tiles.js` style), not iOS. Owner: `@backend-data`. Not urgent.
...
- **Phase 5**: iOS launch — **Swift native + TestFlight** (decided 2026-05-07; in active development since 2026-05-08). MVP streams W1a, W1.5, W2, W3, **W4, W4.5, W5** are merged to `main`. See "Phase 5 progress" table above for stream status. Next stream: **W5.1** (polish) then W6 (notifications). After: W6 → W7 (banner) → W7.5 ("Park Until X" filter) → W8 (TF1, blocked on Apple Dev approval) → W8.5 (Drive Mode, vision-focused port per `docs/drive-mode-scope-spec.md`) → W9 (TF2). v1.0 App Store launch ships with Drive Mode included. Post-MVP follow-on phases: threat tracker UI, zone chat, Smart Move recommendations, address search, snow emergency / NYC 311 API, paywall + StoreKit (see `docs/business-model.md`).
