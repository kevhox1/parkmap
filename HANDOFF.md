...
- **Phase 5**: iOS launch — **Swift native + TestFlight** (decided 2026-05-07; in active development since 2026-05-08). MVP streams W1a, W1.5, W2, W3, **W4, W4.5** are merged to `main`. See "Phase 5 progress" table above for stream status. Next stream: **W5** (pin drop, spec needs to be written). After: W5 (pin) → W6 (notifications) → W7 (banner) → W7.5 ("Park Until X" filter) → W8 (TestFlight, blocked on Apple Dev approval). Post-MVP follow-on phases (5b/5c): port Drive Mode (turn-by-turn, voice, heading-up rotation, parking-aware route selection), threat tracker UI, zone chat, Smart Move recommendations, address search, snow emergency / NYC 311 API, paywall + StoreKit (see `docs/business-model.md`).
