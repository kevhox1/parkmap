...
- **Color threshold change (W4.5).** Spec at `docs/ios-color-threshold-spec.md`. Two open questions at §0 awaiting Kevin's decision (default persona; whether Option C toggle is v1.0 scope).
...
- **Phase 5**: iOS launch — **Swift native + TestFlight** (decided 2026-05-07; in active development since 2026-05-08). MVP streams W1a, W1.5, W2, W3, **W4** are merged to `main`. See "Phase 5 progress" table above for stream status. Next stream: W4.5 (color threshold, awaiting Kevin's §0 answers) OR W5 (pin drop, spec not written yet). After: W5 (pin) → W6 (notifications) → W7 (banner) → W8 (TestFlight, blocked on Apple Dev approval). Post-MVP follow-on phases (5b/5c): port Drive Mode (turn-by-turn, voice, heading-up rotation, parking-aware route selection), threat tracker UI, zone chat, Smart Move recommendations, address search, snow emergency / NYC 311 API, paywall + StoreKit (see `docs/business-model.md`).
...
- **Specs live at the repo root and `docs/`.** Key docs:
  ...
  - `docs/ios-color-threshold-spec.md` — W4.5 spec (locked 2026-05-11). Proposes lowering `nearFutureWindow` from 24h to 6h for the green-comfortably semantic. Two open questions at §0 awaiting Kevin.
...
- **PR #16 merged** as squash commit `c713241`. 5 source commits squashed: initial W4 build + fix-pass-1 (a11y + dismiss bugs) + **rendering refactor** + fix-pass-2 (overlay Z-order + DispatchQueue.main.async threading + new HP-13 test) + a11y sort-priority. Branch `ios/w4-block-detail` deleted.
...
- **`@tech-lead` also produced `docs/ios-color-threshold-spec.md`** (queued as **W4.5**, NOT merged). Recommendation: lower `nearFutureWindow` from 24h to 6h to default the green-comfortably semantic to the short-stay-visitor persona. Surfaced when Kevin noted that an ASP block with cleaning in 5 hours should read as green ("I'll be gone by then") not orange ("warning"). Two open questions for Kevin at §0 before code starts (default persona; whether persona-toggle is v1.0 scope).
