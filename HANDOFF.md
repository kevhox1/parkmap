...
### 2026-09-20 — ✅ REGULARS BACKEND DRAWER-COMPLETE: PR #114 merged (`e49829b8`) — the head-start delay mechanism. Migrations 07/08/09 + two Edge Functions all drafted, QA'd, and held for ONE ceremony.

PR #114 rewrites the live community-push trigger's WHEN clause (leaving_soon-with-head-start no longer zone-pushes at insert; 13-case truth table proved zero drift elsewhere) + `internal.sweep_leaving_soon_zone_push()` (30s pg_cron — the '30 seconds' string form, spec's pseudocode syntax was invalid; FOR UPDATE SKIP LOCKED race-safety; expired-before-elapse skipped forever per honest-exclusivity). QA two-pass: pass 1 live-exploited the sweep being anon-invokable (public-schema PUBLIC-EXECUTE default) → moved to internal schema per repo convention; pass 2 MERGE with a throwaway-role isolation proof + a one-line live ACL check added to Kevin's ceremony. THE CEREMONY DOC is now the 9-step checklist in docs/regulars-roadmap.md's "Kevin's ceremonies" section (single source; QA-authored). Backend QA totals for Regulars: 4 real catches across 3 PRs, all pre-production. Critical path is now pure iOS: S5 (PR #113, QA-cleared) awaits Kevin's 1407/1407 Mac gate; S7+ (invite/QR UI, handoff surfaces) follow. Brooklyn spec still awaits Kevin's three ratifications.
...
Kevin ruled the drive-test gate satisfied (a week of NYC field use on flag-ON build 20) and chose full launch. PR #109 (`e6c728be`): one boolean + the 4 dark-ship guard tests transformed into launched-world assertions — 4-file diff, QA APPROVE, launch gate 1365/1365 WITH THE FLAG COMMITTED TRUE (first time ever; no sed, no revert — that entire ritual is now retired). Build 22 (`4949789c`) archived from main and uploaded 2026-09-14; awaiting TestFlight processing → external review. Externals jump from build 19 to the full stack at once: crew feed, live reports, 41-zone map (live in prod since S14's ceremony), spot handoff, identity/rep, push relevance loop, My Car rules display. S1–S14 + flip: every roadmap row closed. What remains open is the small stuff, deliberately excluded from the launch build for clean diagnostics: polish-2 (#21 Apple-Maps-style drive puck, #12② sweeper-chip affordance, #13 token double-POST, 2 stale-comment nits from PR #109 QA) dispatched as the build-23 opener; then #22 report tap-to-place (spec-first), #18 overnight nudge, backburnered geometry family (#7/#9/#10/#11). NEW STANDING CONTEXT: communityEnabled=true is the shipped state — "flag-off" no longer exists as a test target; suite expectation is a single 1365/1365 run.

### 2026-09-13 (later) — 🏁 S14 COMPLETE: THE 41-ZONE MANHATTAN MAP IS LIVE IN PRODUCTION. The hero roadmap's build phase is DONE.

Migration 06 applied by Kevin (42 rows verified from the VPS via anon curl — 41 zones Battery Park→Inwood + the retired soho-les id, zero dupes). Sim cold-relaunch confirmed the fetch-at-launch pipeline end to end: 8 nearest-first chips (home pinned with a house icon) + the More search sheet listing all 41. Client had merged first (PR #108, suite 1365) and handled both worlds — the apply was a pure data flip, zero client or server code change, exactly as the S14 spec's pre/post matrix promised. With this, EVERY session S1–S14 on the Community 2.0 roadmap is merged and verified. What remains before external users see it: the NYC drive test → the communityEnabled flip (4 named guard tests to adjust in the flip PR). Immediate queue while that waits on Kevin's schedule: polish-2 session dispatched (#21 Apple-Maps-style drive puck, #12-2 sweeper-chip selected-state affordance, #13 token double-POST guard), then the #22 report tap-to-place spec, then #18 overnight nudge.

### 2026-09-13 — ✅ POLISH SESSION MERGED (PR #107 `e36ef063`): thick lines, first-press long-press, blue car everywhere. Suite 1314→1338.

Four items, one PR, one gate: #19 curb widths (browse 3→4.5/metered 6.0, NEW drive-mode step-up 8.0/10.5 via centralized `CurbLineWidth` — QA pass-1 BLOCKED a real miss where the drive step-up never applied on toggle because MapKit caches renderers; fixed by reusing the 60s-timer's `rebuildOverlays` path in the drive-mode funnel, pass-2 verified toggle-only + no re-entrancy). #17b legacy long-press double-press ROOT-CAUSED at last: `handleLongPress` unconditionally rewrote `selectedSegmentID`/`activeSheet` even at no-op values (ActiveSheet not Equatable → no short-circuit) and the resulting view invalidation raced the confirmationDialog's UIKit presentation — second press worked only because state had settled; fixed with a pure reassignment guard. Kevin's gate: first-press worked 3/3. #20 blue-circle car marker replaces the black keyhole everywhere (tentative=dimmed, parked=solid; legend copy updated). Status-line dedupe (identical-text suppression, mixed-block divergence preserved by real-engine test). Gate 2026-09-13: 1338/1338 flag-off, a–f green (metered dedupe case unverifiable live — Sunday, meters off — covered by 7 unit tests). First-try compile. NEW OPEN QUESTION from Kevin at gate: "the black arrow chevron sign is weird, match it to what Maps uses" — ambiguous which arrow (drive puck is location.north.fill/systemBlue; compass is Apple's MKCompassButton; base-map one-ways are Apple's) — awaiting Kevin's pointer before logging. NEXT: S14 execution (fetch-at-launch zones + nearest-first picker + Kevin applies migration 06), then NYC drive test → flag flip.

### 2026-09-12 — ✅ CORE-PARKING SESSION MERGED (PR #106 `a07bc014`): My Car answers "free until when," ships flag-off. Suite 1282→1314.

Open-item #16 + #17-residual in one PR: the My Car sheet now shows a "Free until Monday 9:30 AM"-style status line (same `nextRestriction` family the reminders use — sheet can never disagree with a notification), a collapsible `RuleRow` sign-details list, and an ASP-suspended capsule — all NOT flag-gated, live for every user next build. Kevin's gate ran ON the real Rosh Hashanah suspension weekend: "Free until Monday" correctly leapt the suspended Sat/Sun, and the capsule showed live. Plus the flag-gated tentative blue-car pin at the long-press point. One QA BLOCK round did its job: the status line originally said "Free — no restrictions here" on metered-only blocks (~6% of rule-bearing segments — `nextRestriction` intentionally skips METERED; FT-9 bug class reintroduced) — fixed with meter-aware fallback + regression tests before Kevin ever saw it. One compile round (new failure class #6, banked in memory: bare `Category` is ambiguous vs ObjC runtime in test targets — qualify `WePark.Category`). Gate findings spawned: #17b (legacy dialog STILL double-presses — a second cause beyond the fixed `.began` bug; pre-existing, not a #106 defect), #19 (curb lines too thin — first external-tester feedback, wants a Drive-Mode-specific width), #20 (Kevin: promote the blue car marker to THE parked-car icon), plus the headline/status-line duplication dedupe. All four bundled into ONE polish session, dispatched 2026-09-12, gate before S14. Also logged #18 (night "sea of green" vs overnight parking — Park Until nudge is the ruled-path candidate, after S14).

### 2026-09-11 — ✅ S13c COMPLETE: hero-parity fix list + garage-savings + long-press park, all merged (PR #105 `767ded10`). Suite 1221→1282.

The whole S13a/S13b tail is closed in one PR: all 13 audit fix items (docs/design/community-2.0-final-parity-audit.md), the garage-savings stat (Option A, "$X back in your pocket this month — no garage needed"), AND open-item #17 folded in mid-gate at Kevin's direction (long-press two-attempt bug — popup presented on `.began` under the still-down finger; now `.ended` — plus the flag-gated slim "Park here?" card, flag-off dialog untouched). Kevin's live gate caught 3 bugs static QA couldn't: the away-note's home zone was value-threaded once at sheet-open and never refreshed on chip taps (now computed live), the zone chips overlapped "New to parking?" on an empty feed (max-height → fixed-height slot), and the new card collided with the Report pill/"?" chrome (chrome now hides while the card is up). Also settled: the 1221-vs-1246 test-count scare was a branch-vs-main measurement artifact (QA traced it, docs/qa/pr105-community-s13c-parity.md); flag-on guard-failure count is now 4, named in the roadmap, reconcile by name not number. Two ops lessons banked as memories: never put derivedDataPath in an iCloud-synced folder (codesign "detritus" race — Kevin's Desktop), and the sim's KVS parked-car state survives app reinstall (a resurrected 9-day test car is what made the garage card show $154 — math hand-verified correct). New open items born this session: #16 (My Car sheet must show parked-blockface rules + ASP status — next session, core value, not flag-gated), #17 residual (tentative pin at long-press point → #16 session), S14 picker requirement (nearest-zones-first per Kevin). NEXT: #16+#17-residual core-parking session, then S14 execution (fetch-at-launch zones + picker + Kevin applies migration 06), then the NYC drive test → flag flip.
