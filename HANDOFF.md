...
- **Viewport polish — hide overlays at wide zoom + auto-center at launch.** Kevin caught during W6.1 smoke (2026-05-16): at wide-zoom-out covering Manhattan + Brooklyn, the parking overlays render in patchwork (only some streets colored) because `TileLoader` LRU evicts tiles as it overflows the 200-cap. Looks broken. Spec at `docs/viewport-polish-spec.md` covers two-part fix: (A) lower `polylineHideSpanThreshold` `0.1 → 0.04` so overlays cleanly hide at wide zoom; (B) auto-center on launch to user location (if authorized + within 25 km of Manhattan) OR parked car coordinate (if deep-link tap), fall back to `manhattanCenter`. ~1 engineer session, ~10 files touched. **Two open decisions** for Kevin to answer before engineer starts: OD-1 fresh GPS fix vs cached, OD-2 25 km radius vs 15 km. Owner: `@ios-engineer` after Kevin answers OQs. Defer dynamic tile loading + bigger LRU cache to post-W8 real-device measurement.
...
- **Phase 5**: iOS launch — **Swift native + TestFlight** (decided 2026-05-07; in active development since 2026-05-08). MVP streams W1a, W1.5, W2, W3, **W4, W4.5, W5, W5.1, W6, W6.1, W7** are merged to `main`, plus 3 PRs against tile / SW pipelines (#21 + #22 intersection-overshoot fix, #23 SW cache bump). See "Phase 5 progress" table above for stream status. Next stream: **viewport-polish** (small UX PR — spec at `docs/viewport-polish-spec.md`, ~1 engineer session, hides overlays at wide zoom + auto-centers on user/parked-car at launch) or **W7.5** ("Park Until X" filter). After: W7.5 → W8 (TF1, blocked on Apple Dev approval) → W8.5 (Drive Mode, vision-focused port per `docs/drive-mode-scope-spec.md`) → W9 (TF2). v1.0 App Store launch ships with Drive Mode included. Post-MVP follow-on phases: threat tracker UI, zone chat, Smart Move recommendations, address search, snow emergency / NYC 311 API, paywall + StoreKit (see `docs/business-model.md`).
