...
| S14 | **Manhattan zone map** (added 2026-09-06, Kevin's directive) | Neighborhood zones across Manhattan (reasonable sizes, clear boundaries snapped to major streets): migration seeds + client fetch-at-launch replacing hardcoded `CommunityZoneBounds` + a scalable zone picker (3 chips don't scale to 20+). Sequence: after S13c, recommended BEFORE the external flag flip. ~2 sessions (data design + client) | Apply migration; visual check of new zone picker |
