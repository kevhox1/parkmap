...
|---|---|---|---|
| S1 | **Phase 0 build** *(dispatched 2026-08-26)* | @backend-data writes the migration per spec §2 (enum additions, `position_fraction`/`leaving_minutes`/`claimed_by`, `claim_pin` RPC, rep triggers +5/+2/+1, rate-limit generalization, `device_push_tokens`, 3 zone-box seeds, server-derived `expires_at`, hygiene sweep) + curl test script. PR only — **never applies anything** | — |
| S2 | **Phase 0 QA → merge → prod** | @qa-verifier pass, fixes, merge | Apply migration in dashboard + run test script (~30 min) |
...
| S6 | **Phase 2a → merge** | Confirm-the-street (W5 pattern reuse) + closure tile → existing `BlockRestrictionReportSheet`. Reuse-heavy, QA same session | Mac: test run |
