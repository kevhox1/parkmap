...
| S13a | ✅ **Map chrome → MERGED `133c459f`** (2026-09-06, PR #102) | Report pill (long-press stays as shortcut) + "?" map-key legend (curb copy byte-verbatim; 4 pin rows per shipped markers) + zone-boundary overlay (renderer diff 100% additive) + double-POST fold-in. Suite 1183→**1208**, first-try compile again | ✅ Gate 2026-09-06: 1208/1208 · flag-off byte-identical 📸 · pill/"?"/legend/box vs screenshots 01-03 ✓ · drive-mode gating ✓ (the "pill" sighting was FT-11's own drive Report button — not a bug). **Kevin's rulings logged (open-items #15):** own-box-only, label only when genuinely yours, hidden in drive mode → S13c; zone rebalance (LES oversized, maybe more zones) = separate data task |
| S13b | ✅ **Block detail + chat → MERGED** (2026-09-08, PR #103) | Screenshot-07 block detail (legality-color header, LIVE ON THIS BLOCK, BLOCK CHATTER) + the chat WRITE PATH (zero new SQL — RLS already permitted; server validates author_id regardless of client). One Mac-gate round: flaky TEST (over-permissive mock racing a background fetch), impl correct. Suite 1208→**1221** | ✅ Gate 2026-09-08: 1221/1221 · block detail 📸 · chat sent from both surfaces · **rep arithmetic exact: sim-Moose 10→12 via two +1 chat awards** (two-Moose confusion resolved: phone profile is separate, 15). Chatter-proportions nit logged → S13c |
| S13c | ✅ **Hero-parity pass → MERGED `767ded10`** (2026-09-11, PR #105) | Audit (`docs/design/community-2.0-final-parity-audit.md`, 2026-09-08): 13-item fix list — ALL shipped (zone-box own-only/genuinely-yours/drive-hide per #15①②; icon palette unified teal/cyan; "Not sure" heading chip + visible one-way inference; cleaning-truck pill removed; "0%"→"—"; chatter density + Realtime live-update; confirm-street scroll; HEADING TOWARD label; shared SweptBadgeView; 1000-char test; away-note) + **garage-savings stat** (Option A, `GarageSavingsService`, $0.70/hr from Parking 101's $500/mo, device-local, ET month-reset, copy "$X back in your pocket this month — no garage needed" — Kevin saw $154 live from a 9-day resurrected test car, math hand-verified) + **open-item #17 folded in at Kevin's direction**: long-press two-attempt bug root-caused (popup presented on `.began` while finger still down — same touch-up dismissed it; fix = present on `.ended`) + flag-gated `LongPressParkConfirmCard` slim popup (flag-off dialog byte-identical) + chrome-hide while card is up. Gate found 3 live bugs QA's static pass couldn't (away-note stale value-threading — home zone never refreshed on chip tap; chips overlapping "New to parking?" on empty feed — max-height not fixed-height; card/chrome collision). QA: `docs/qa/pr105-community-s13c-parity.md` (also reconciled the 1221-vs-1246 count scare: branch-vs-main artifact, not a gap). Suite 1221→**1282** | ✅ Gate 2026-09-11: flag-off **1282/1282** · flag-on 1278+4 named dark-ship guards (count updated 3→4, reconcile by NAME) · smoke: zone box follows car not viewport ✓ · drive-mode hide ✓ · garage card ✓ · away-note ✓ ("You're browsing SoHo — your home square is Nolita") · icon parity ✓ · Not-sure chip ✓ · long-press first-attempt sticks ✓ "worked perfectly" · flag reverted ✓. Residual: tentative pin at press point → #16 session |
| S14 | **Manhattan zone map — ✅ PROPOSAL RULED by Kevin 2026-09-08 ("41 zone ruling is good for now")** | Proposal + draft migration merged (PR #104: 41 zones Battery Park→Inwood, ids stable, `docs/community-2.0-manhattan-zones.md` + preview HTML + `supabase/06-manhattan-zones.sql` DRAFT — DO NOT APPLY YET). Execution session after S13c: client fetch-at-launch replacing hardcoded `CommunityZoneBounds`, scalable zone picker, QA, THEN Kevin applies 06 | Apply migration 06 at execution; picker visual check |
