...
| 15 | **Zone-box display rules + rebalance (Kevin's rulings, 2026-09-06 S13a gate)** | VPS (S13c + a data task) | ① Show ONLY the user's own zone box (parked car's zone, else current-location zone) — never all three; "YOUR SQUARE" label only when genuinely theirs; browsing other boards stays via feed chips · ② hide the box in drive mode (minimal chrome) · ③ **Zone geometry rebalance**: LES box is oversized vs Nolita/SoHo; Kevin may want more/smaller zones — a zones-table UPDATE + CommunityZoneBounds client sync (they must ship together; client hardcodes boxes — the known OQ-1 tradeoff). Kevin to rule on the zone list; ①② land in S13c |
