...
| 19 | **Curb lines too thin — external-tester feedback** (via Kevin, 2026-09-12: "friendly/live feedback… the lines are not thick enough… especially in the drive mode") | VPS (small tuning session, PAIR WITH #17b) → Mac visual gate | First externally-sourced UI feedback of the project. Curb polyline `lineWidth` lives in the map renderer (`MapViewRepresentable`) — likely zoom-dependent. Work: bump base thickness; consider a LARGER Drive-Mode-specific width (windshield viewing distance ≠ in-hand browsing — ties to the standing S6 sunlight-legibility concern, which no simulator can gate). Pure rendering tune, flag-off surface, no logic. Gate must be visual on-device/sim at several zooms + drive mode; watch overlap at dense intersections when widths grow |
| 20 | **Promote the blue car marker to be THE parked-car map icon** (Kevin, PR #106 gate 2026-09-12: "use this little blue car icon as the actual [parked] car icon… it's more fitting and it looks better") | VPS (fold into #19/#17b polish session) | PR #106's tentative-pin marker (blue circle + car glyph, `PendingParkPinAnnotation` styling) replaces the current black keyhole parked-car marker everywhere the parked car renders. Marker-style swap in `MapViewRepresentable` — visually distinguish tentative (e.g. slight transparency/dashed ring) vs parked (solid) so the confirm transition still reads. Note: map-key legend copy says the car pin is blue-not-black in its footer ("describe reality" rule) — verify legend text/entry still matches after the swap. Also sanity-check the S13c dedupe follow-up (headline vs status-line duplication) lands in the same session |
| 17b | **Legacy long-press dialog STILL double-presses — second cause, not the `.began` bug** (Kevin, PR #106 gate 2026-09-12) | VPS (pair with #19 session) | PR #106's flag-off check confirmed the legacy three-button `confirmationDialog` still "needs double pressing" — with the S13c `.ended` gesture fix verifiably live for both paths (recognizer fires `.ended`-only, unconditionally, MapViewRepresentable ~2665). So the flag-on card is fixed but the legacy dialog has a SECOND dismissal mechanism (`confirmationDialog`-specific: presentation-boolean race with map selection/annotation state is the suspect family — this is the ORIGINAL #12① bug, never actually root-caused, only superseded). Matters because ALL users incl. Kevin run flag-off until the flip. NOT a #106 defect (dialog required byte-identical; bug is pre-existing). Fix in the #19 session; dies anyway at flag flip, but flip timing is drive-test-gated |
