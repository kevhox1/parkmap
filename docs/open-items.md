...
| 21 | **Drive-Mode heading arrow: restyle to match Apple Maps' navigation puck** (Kevin, PR #107 gate 2026-09-13: "the black arrow chevron sign is weird — match it to whatever Maps uses") | VPS (small visual item — ride along with the next map-touching session) | Current: bare SF `location.north.fill` at 28pt semibold tinted `.systemBlue` (`MapViewRepresentable` ~2242) — over the dark map it reads as a flat dark arrow, not a nav puck. Target: Apple-Maps-style course puck — the arrow/chevron inside a filled circular badge with a white ring/shadow (blue circle, white chevron), so it reads as "you" at windshield distance. Same annotation-view path, purely visual; keep the identity-rotation convention (camera rotates, puck doesn't — Build-7 TF2-3 #1 doc comment). Gate: visual in Drive Mode, day + dark |
