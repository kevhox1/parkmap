...
| S1-follow-up | ✅ **DONE-PENDING-QA** (2026-09-18, `supabase/09-regulars-zone-push-delay.sql` + companion test script, DRAFT — DO NOT APPLY) — **this is the gate-clearing artifact S3's own cell below and open-items #23 point to** | `@backend-data` | The deferred §2.8 piece from S1's SCOPE NOTE, now built: `supabase/09-regulars-zone-push-delay.sql` (1) rewrites `04-community-push-trigger.sql`'s live `pins_invoke_send_community_push` trigger's WHEN clause only (the trigger FUNCTION body is untouched — zero lines of `internal.invoke_send_community_push()` change) so a `leaving_soon` pin posted WITH a positive `regulars_head_start_seconds` no longer fires the immediate zone-wide push; every other case (every other `pin_type`, and a `leaving_soon` pin with a null/zero head start) is provably byte-identical to today's live behavior — see the file's own Section A proof-by-inspection comment; (2) adds `sweep_leaving_soon_zone_push()`, a `SECURITY DEFINER` function that finds a head-start `leaving_soon` pin whose head start has elapsed (`created_at + regulars_head_start_seconds <= now()`), is not yet zone-pushed, and has not already expired/resolved (a pin that expires or gets resolved before its head start elapses is skipped FOREVER, by construction — the honest-exclusivity ruling, spec §1.2a/§0 decision 8 — no special-case code needed, it falls out of `expires_at`/`resolved_at` being one-way monotonic state), fires the same, unmodified `send-community-push` Edge Function for it, and stamps `zone_pushed_at` immediately after the dispatch attempt (success or failure alike — a deliberate one-shot, fire-and-forget choice matching `04`'s own existing no-retry posture, stated explicitly in the file rather than left implicit); race-safety against overlapping sweep runs is `SELECT ... FOR UPDATE SKIP LOCKED` (found and fixed one real bug live: Postgres rejects a schema-qualified name in `FOR UPDATE OF`, i.e. `for update of public.pins` errors — must be the unqualified `for update of pins`); (3) schedules that sweep on `pg_cron` — **30-second cadence achieved as specced**, but via the correct syntax: pg_cron's dedicated `'30 seconds'` schedule-string form (supported since pg_cron 1.5; Supabase ships 1.6.4+, already enabled/load-bearing on this project per `02d-ingest-cron.sql`), NOT the spec §2.8 pseudocode's literal `'*/30 * * * * *'` 6-field string, which is not valid pg_cron syntax at all — flagged as a spec-pseudocode correction, not a cadence downgrade; no 1-minute fallback was needed. **One named, deliberate deviation from §2.8's literal pseudocode:** the sweep does NOT pick up null-head-start `leaving_soon` pins via `coalesce(regulars_head_start_seconds, 0)` as the pseudocode sketched — those pins keep firing the immediate trigger, unchanged, exactly as today (zero added latency for the common case, and a smaller/cheaper candidate set for the first sub-minute cron job this repo has ever run); net product behavior for a null-head-start pin is identical either way, this is a mechanism choice, flagged for `@qa-verifier`/`@tech-lead` to confirm against spec intent rather than silently diverging. **Local validation (real Postgres 16, this session):** applied the full chain `01→02→02b→02c→02d→02e→02f (minus Storage)→02g→03→04→05→06→07→09→08` clean, zero errors, and idempotent (07/09/08 all re-applied cleanly a second time back-to-back). Live-exercised on a scratch instance: a fresh head-start `leaving_soon` pin has `zone_pushed_at` null immediately after insert (confirmed both by direct observation of the column AND by the absence of a `send-community-push` fail-open log line at that instant, present only for the no-head-start/`open_spot` control pins); after backdating that pin's `created_at` past its head start and running the sweep, `zone_pushed_at` becomes set and the fail-open log line appears; a THIRD sweep run afterward leaves `zone_pushed_at` byte-identical (no double-push, no-op confirmed); a control pin whose `expires_at` was set into the past (simulating "departure window shorter than head start") is never touched by the sweep across three separate runs — `zone_pushed_at` stays null forever, the honest-exclusivity behavior observed live, not just reasoned about. Companion `supabase/09-regulars-zone-push-delay-test.sh` (curl/anon-key-only, mirrors `04`/`08`'s own test-script shape) — notably stronger than `04`/`08`'s own scripts on this one axis: `pins_with_author` already exposes `zone_pushed_at` as a plain pollable column, so Tests 1-5 are fully automatable over anon-key REST alone (no SQL-Editor-only step required to observe the delay/no-op/never-pushed properties themselves, only to confirm the underlying `net._http_response`/Edge-Function-log detail) — Test 6 (honest exclusivity, slow, ~9 min, skippable via `RUN_SLOW_EXPIRY_TEST=false`) is the one exception, needing a pin to actually cross its own `expires_at`. **Apply order: `07` → `09` (this row) → `08`**, not merely "no ordering requirement between 08/09" — applying `09` before `08`/before deploying `send-regular-push` is the actual fix for the PR #112 QA finding (closes the window where a visible Regulars push and an immediate zone push could both fire for the same pin); see the file's own APPLY ORDER section for the full reasoning. **Must land and be applied before Kevin deploys `send-regular-push`/applies `08-regulars-push-trigger.sql`** — see S3's own gate cell below, now updated to name this file directly. | — |
