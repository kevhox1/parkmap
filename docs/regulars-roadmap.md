...
| S1-follow-up | ✅ **MERGED `e49829b8`** (2026-09-20, PR #114, two-pass QA — pass 1 caught the anon-invokable sweep, fixed via internal-schema move; pass 2 MERGE with the mechanism isolated via throwaway-role proof) | `@backend-data` | The deferred §2.8 piece from S1's SCOPE NOTE, now built: `supabase/09-regulars-zone-push-delay.sql` (1) rewrites `04-community-push-trigger.sql`'s live `pins_invoke_send_community_push` trigger's WHEN clause only (the trigger FUNCTION body is untouched — zero lines of `internal.invoke_send_community_push()` change) so a `leaving_soon` pin posted WITH a positive `regulars_head_start_seconds` no longer fires the immediate zone-wide push; every other case (every other `pin_type`, and a `leaving_soon` pin with a null/zero head start) is provably byte-identical to today's live behavior — see the file's own Section A proof-by-inspection comment, independently confirmed by QA's own 13-case truth table diffed against a pre-09 control DB; (2) adds `internal.sweep_leaving_soon_zone_push()` (schema corrected post-QA — see below), a `SECURITY DEFINER` function that finds a head-start `leaving_soon` pin whose head start has elapsed (`created_at + regulars_head_start_seconds <= now()`), is not yet zone-pushed, and has not already expired/resolved (a pin that expires or gets resolved before its head start elapses is skipped FOREVER, by construction — the honest-exclusivity ruling, spec §1.2a/§0 decision 8 — no special-case code needed, it falls out of `expires_at`/`resolved_at` being one-way monotonic state; QA independently verified BOTH the expires_at and resolved_at cases, the latter not in the builder's own test matrix), fires the same, unmodified `send-community-push` Edge Function for it, and stamps `zone_pushed_at` immediately after the dispatch attempt (success or failure alike — a deliberate one-shot, fire-and-forget choice matching `04`'s own existing no-retry posture, stated explicitly in the file rather than left implicit); race-safety against overlapping sweep runs is `SELECT ... FOR UPDATE SKIP LOCKED` (found and fixed one real bug live: Postgres rejects a schema-qualified name in `FOR UPDATE OF`, i.e. `for update of public.pins` errors — must be the unqualified `for update of pins`; QA independently reproduced this mechanism with a genuine two-session interleaving, not just a timing guess); (3) schedules that sweep on `pg_cron` — **30-second cadence achieved as specced**, but via the correct syntax: pg_cron's dedicated `'30 seconds'` schedule-string form (supported since pg_cron 1.5; Supabase ships 1.6.4+, already enabled/load-bearing on this project per `02d-ingest-cron.sql`), NOT the spec §2.8 pseudocode's literal `'*/30 * * * * *'` 6-field string, which is not valid pg_cron syntax at all — flagged as a spec-pseudocode correction, not a cadence downgrade; no 1-minute fallback was needed; QA independently confirmed this against pg_cron's own CHANGELOG (v1.5.0) and Supabase's own Cron quickstart doc, and flagged one residual unverified item (a Postgres-patch-version prerequisite for sub-minute cron, "15.1.1.61+" per Supabase's docs — see the consolidated ceremony checklist above, step 8). **One named, deliberate deviation from §2.8's literal pseudocode:** the sweep does NOT pick up null-head-start `leaving_soon` pins via `coalesce(regulars_head_start_seconds, 0)` as the pseudocode sketched — those pins keep firing the immediate trigger, unchanged, exactly as today (zero added latency for the common case, and a smaller/cheaper candidate set for the first sub-minute cron job this repo has ever run); net product behavior for a null-head-start pin is identical either way, this is a mechanism choice — QA's own nit: this implementation is strictly *faster* for that case than the literal pseudocode, not merely equivalent, which slightly undersells itself in the file's own SCOPE NOTE. **QA pass 1 findings, both fixed same day:** 🔴 **Finding #1 (blocking, live-reproduced): the sweep function was originally `public.sweep_leaving_soon_zone_push()` with only an ADDITIVE `grant ... to postgres`**, which does not revoke Postgres's default PUBLIC EXECUTE grant — QA confirmed live that `anon`/`authenticated` could call it directly via `POST /rest/v1/rpc/sweep_leaving_soon_zone_push`, a real production risk (unauthenticated-adjacent full-table-scan trigger + forced early dispatch, at will, for every user's pins). Fixed by moving the function to `internal.sweep_leaving_soon_zone_push()` — QA's own recommended fix (matches `internal.invoke_send_community_push()`/`internal.invoke_send_regular_push()`/`internal.invoke_film_permit_ingest()`'s established, least-novel "PostgREST doesn't expose this schema" convention, rather than bolting on an explicit `REVOKE`). Re-verified live post-fix: `set role anon`/`authenticated` both now fail with `permission denied for schema internal` (schema-level `USAGE` was never granted to those roles, matching every other `internal.*` helper), `postgres` (the cron-invocation role) still succeeds, and the full sweep behavior (elapse-triggers-push, no-op on repeat, skip-forever) is unchanged post-move. Test script gained an explicit `Test 0` regression guard (expects HTTP 404 from PostgREST for both anon and authenticated). 🟡 **Finding #2: this file's own "Kevin's ceremonies" section (above) had no distinct "apply 08" bullet** — fixed by replacing that section with QA's own 9-step consolidated checklist verbatim, per its own recommendation, rather than re-paraphrasing. **Local validation (real Postgres 16, both the original session and the QA-fix re-validation):** applied the full chain `01→02→02b→02c→02d→02e→02f (minus Storage)→02g→03→04→05→06→07→09→08` clean, zero errors, and idempotent (07/09/08 all re-applied cleanly a second time back-to-back) — re-confirmed after the schema move. Live-exercised on a scratch instance, post-fix: a fresh head-start `leaving_soon` pin has `zone_pushed_at` null immediately after insert; after backdating that pin's `created_at` past its head start and running `internal.sweep_leaving_soon_zone_push()`, `zone_pushed_at` becomes set and the fail-open log line appears; a THIRD sweep run afterward leaves `zone_pushed_at` byte-identical (no double-push, no-op confirmed); a control pin whose `expires_at` was set into the past is never touched across three separate runs — `zone_pushed_at` stays null forever. Companion `supabase/09-regulars-zone-push-delay-test.sh` (curl/anon-key-only, mirrors `04`/`08`'s own test-script shape, plus the new Test 0 above) — Tests 0-5 are fully automatable over anon-key REST alone; Test 6 (honest exclusivity, slow, ~9 min, skippable via `RUN_SLOW_EXPIRY_TEST=false`) is the one exception, needing a pin to actually cross its own `expires_at`. **Apply order: `07` → `09` (this row) → `08`** — applying `09` before `08`/before deploying `send-regular-push` is the actual fix for the PR #112 QA finding (closes the window where a visible Regulars push and an immediate zone push could both fire for the same pin); see the file's own APPLY ORDER section, and the consolidated 9-step ceremony checklist above, for the full reasoning. **Must land and be applied before Kevin deploys `send-regular-push`/applies `08-regulars-push-trigger.sql`** — see S3's own gate cell below and the ceremony checklist above. Pending `@qa-verifier`'s re-pass of the two fixes before this row can flip to fully MERGED. | — |
...
| S7 | 🟡 **PR #115 open** (`[COMPILE-UNVERIFIED]`, branch `ios/regulars-s7-invites`) — pending Kevin's Mac gate + `@qa-verifier` | `@ios-engineer` | iOS UI: `RegularsSettingsView.swift` (Regulars list via `fetchEdges` + handle/avatar resolution, `.swipeActions` remove/block, "Add a Regular"), `RegularInviteView.swift` (QR via CoreImage `CIQRCodeGenerator` + `ShareLink`, 10-min countdown via `TimelineView`, Cancel/regenerate, redemption-detected polling), `RegularInviteRedemptionView.swift` (new — the redeem-side confirm sheet, all four RPC result states + a transport-error state), `WeParkApp.swift` `.onOpenURL` + a new `regularsService` app-lifetime singleton (mirrors `authService`'s AC-A5 posture), `Info.plist` `CFBundleURLTypes` for `wepark://` (file edit made directly per the dispatch; Kevin's gate confirms Xcode picks it up), `SettingsView.swift` row wiring (gated on `regularsSettingsRowVisible()`). New pure-logic file `Services/RegularsInviteRouting.swift` (`RegularsInviteLink` build/parse/`resolveRedemptionToken`, `InviteCountdown`, `RegularInviteRedemptionCopy`) plus three `RegularsService` additions not wired in S5 (`removeRegular`/`cancelInvite`/`fetchInvite`) and a new `fetchProfiles(ids:)` public-`profiles` read (`RegularProfileSummary`, `Models/Regular.swift`) for handle/avatar display — none of these were in S5's own dispatch list, flagged there as deferred to S7/S9. **Known, flagged gap:** the pre-redemption confirm sheet does NOT name the inviter (spec §3.2's own sketch assumes it can) — `regular_invites_select_own` restricts SELECT to the creator, so a redeeming session has no RLS path to the inviter's identity before calling the RPC; copy is honest-but-generic instead of fabricated, flagged in the PR for the orchestrator. **QA pass 1** (`docs/qa/pr115-regulars-s7.md`, `fb1352ca`): MERGE-PENDING-MAC-GATE, zero blocking findings. One 🟡 fixed on this branch (`9836f167`): "New invite" was orphaning the previous token (left redeemable for the rest of its TTL, unrevoked) — fixed via new `RegularsService.regenerateInvite(previousId:)`, which revokes the outgoing invite BEFORE creating its replacement (retries the revoke once; if both attempts fail, never creates a replacement, so two live invites can't happen through this path even across an app kill). `RegularInviteView`'s initial load, "New invite", and "Try again" now all route through this one method. Also swept three QA nits in the same commit: trailing-slash URL parsing is now explicitly documented + locked in by a test (accepted, not rejected — `URL.pathComponents` already normalizes it away), the countdown's doc comment now states plainly it's device-clock-cosmetic (server enforces the real TTL), and `fetchInvite(id:)` got its own dedicated wire test matching its siblings (39 tests, main → 1446). **LIVE GATE (Kevin, real simulator, flag-on): redemption sheet never presented** — `xcrun simctl openurl` with a valid invite link foregrounded the app but presented nothing, from every app state. **Root cause confirmed, fixed on this branch (`860fbe14`):** the redemption sheet was attached via a SECOND, independent `.sheet(isPresented:)` directly on `ContentView(...)` from `WeParkApp.swift` — `ContentView` already hosts the app's one `.sheet(item: $activeSheet)` presenter, and FT-20 Stream A made `.browseNav` that host's persistent rest state (not "nothing"), so the second sheet never got a presentation slot. Same *class* of bug PR #96 fixed for `.identityPrompt`. Fix: new `ActiveSheet.regularsInviteRedemption(token:)` case, `.onOpenURL` moved from `WeParkApp.swift` into `ContentView.swift` (`routeRegularsInviteRedemption(_:)`, unconditional preemption matching `routePendingDeepLink(_:)`'s own precedent), `WeParkApp.swift`'s standalone sheet/state removed entirely. Flag-off no-op behavior untouched (folds through the same `resolveRedemptionToken` gate). 3 new tests (`RegularsInviteRedemptionActiveSheetTests`) — final count **1449**. Depends on S5 (merged); needs S3/S4 deployed for the live-push half of its gate (built against S5 alone, that half deferred to S13 per the dispatch). | S8 |
...
**The push-pipeline ceremony (07 → 09 → 08 → deploy) is a 9-step checklist, not a bullet-per-file
list** — the version below is copied VERBATIM from `docs/qa/pr114-regulars-delay.md`'s own
"Consolidated ceremony checklist for Kevin" section (2026-09-20, QA pass 1 on PR #114), which was
written specifically to supersede the scattered "ceremony order" prose that had drifted apart across
this file's S1-follow-up/S3 rows and `docs/open-items.md` #23 (QA Finding #2 on that same pass: this
section previously had no distinct "apply 08" bullet, which is exactly the kind of drift this
single-source-of-truth approach is meant to prevent going forward). **Do not re-paraphrase this list
in a future edit — update the QA doc's own list if the ceremony changes, then re-copy it here, so
this section and that one can never silently diverge again.**

1. **Apply `supabase/07-regulars-schema.sql`** via the SQL Editor, if not already applied. Run
   `07-regulars-schema-test.sh`.
2. Confirm `supabase/09-regulars-zone-push-delay.sql`'s own QA sign-off is clear (Finding #1 from
   `docs/qa/pr114-regulars-delay.md` — `sweep_leaving_soon_zone_push()` was originally anon-invokable
   via PostgREST; fixed by moving it to the `internal` schema, `internal.sweep_leaving_soon_zone_push()`,
   matching `internal.invoke_send_community_push()`/`internal.invoke_send_regular_push()`'s own
   established, PostgREST-inaccessible convention).
3. **Apply `supabase/09-regulars-zone-push-delay.sql`.** This rewrites the live
   `pins_invoke_send_community_push` trigger's WHEN clause and adds the sweep + its cron job.
4. **Verify in the SQL Editor, before doing anything else:**
   ...
   ```
   Then insert one real test pin with `regulars_head_start_seconds = 60`, wait ~90s, and confirm
   `zone_pushed_at` becomes non-null and `cron.job_run_details` shows a successful run for this job.
   **Do not proceed to step 5 until this is confirmed** — per the QA report's own partial-state walk,
   this is the one step in the whole ceremony that is load-bearing, not just sequencing hygiene: if
   the sweep silently failed to register while `08` went live anyway, a head-start pin would get its
   immediate Regulars push but never a zone-wide one — a real, silent regression.
5. **Deploy `send-regular-push`** and **redeploy `send-community-push`** (the `_shared/apns.ts`
   extraction from PR #112 only takes effect in production once redeployed).
6. **Apply `supabase/08-regulars-push-trigger.sql`.**
7. **Run `supabase/08-regulars-push-trigger-test.sh`** and `supabase/09-regulars-zone-push-delay-test.sh`
   (the latter's Test 6 is slow/optional — `RUN_SLOW_EXPIRY_TEST=false` to skip).
8. **Confirm the pg_cron sub-minute-scheduling Postgres-version prerequisite** noted in Supabase's own
   Cron docs ("Postgres version 15.1.1.61 or later") is met on this project — a one-time dashboard
   check, not expected to be an issue but not independently verified by any agent so far.
9. (Housekeeping, already done as of this edit) Keep this section and `docs/open-items.md` #23 pointed
   at the QA doc's own list rather than re-duplicating it, so the two can't drift apart again.

Separately, not part of the push-pipeline ceremony above:
