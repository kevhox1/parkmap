...
| S5 | ⬜ Not started | `@ios-engineer` | iOS model/service layer: `Models/Regular.swift` (`RegularEdge`/`RegularInvite`/`PinNote`/`RegularNotice`, the last with `scheduledFor`), `Services/RegularsService.swift`, `AppConstants.regularsEnabled` + the now-**6** guard tests named above. Zero UI. Depends on S1's FILE (not its production apply) — can start in parallel with S3/S4 (different codebases). | — |
