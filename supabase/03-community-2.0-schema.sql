...
-- confirm — same no-double-fire shape as auto_resolve_on_dispute (column-scoped trigger). Fires on
-- every vote mutation for the pin (refresh_pin_vote_counts always sets confirm_count in its UPDATE's
-- SET list, even when the value doesn't change — same "AFTER UPDATE OF column fires whenever the
-- column is in the SET list, not only when its value changes" behavior 02e's own comment documents),
-- but the old/new guard below ensures the actual increment only happens on the 0->1 transition.
...
-- ============================================================
-- §2.8 Rate limiting — generalize the existing pattern to ephemeral crowd reports
-- ============================================================
-- rate_limit_config (02f-block-scoped-restrictions.sql:704-721) is already designed to be retuned
-- by row update, not migration. One more key, scoped to ephemeral crowd reports (enforcement/
-- sweeper/open_spot — leaving_soon is naturally self-limiting, one active pin per parked car, per
-- spec §2.8, so it is not excluded by a pin_type filter here; if a leaving_soon pin happens to also
-- be lifespan='ephemeral' it is harmlessly counted too, since its natural one-per-car cadence never
-- approaches the cap).
...
comment on column public.rate_limit_config.max_rows is
  'Absolute cap on total block-scoped crowd report rows an author may create in window_hours (see '
  '02f''s original comment for the block_scoped_report key''s two-guard reasoning). For the '
  'ephemeral_report key specifically (§2.8), this column is currently UNUSED by design — see '
  'enforce_ephemeral_report_rate_limit() below for why a single guard fully bounds ephemeral crowd '
  'reports (they never carry a report_group_id, so 02f''s "reused fixed report_group_id" bypass '
  'class does not apply here) — seeded per spec §2.8''s literal insert regardless, so a future '
  'second guard can be wired up here without a schema change if abuse patterns ever require it.';

-- enforce_ephemeral_report_rate_limit(): same qualifying-row predicate spec §2.8 specifies
-- (source='crowd' and lifespan='ephemeral'), same shape as 02f's enforce_block_scoped_rate_limit
-- (config-driven thresholds, SECURITY DEFINER, raises 42501 over the cap) — but deliberately a
-- SINGLE guard (count(*) of this author's qualifying rows in the trailing window), not 02f's
...
-- reports (reused report_group_id, delete-then-reinsert). Ephemeral crowd reports have no
...
-- concern, which is far less attractive against a 3-minute-TTL open_spot pin than a
-- days-to-weeks-TTL block closure. If live abuse is observed, port 02f's append-only ledger
-- pattern directly (same author_id/created_at index shape) rather than adding a second live-count
-- guard.
--
...
-- principle insert a non-ephemeral crowd pin, then UPDATE lifespan to 'ephemeral' after the fact
-- (lifespan remains in the anon/authenticated UPDATE grant list — 02f section 2b — since it is a
-- legitimate author-edit field for other reasons), bypassing this guard on that one row. Not closed
-- here to keep Phase 0 scoped to what spec §2.8 asks for ("full body omitted... ports it directly");
-- if this is exploited in practice, widen the trigger to BEFORE INSERT OR UPDATE with the same
-- old/new skip-condition shape 02f uses.
create or replace function public.enforce_ephemeral_report_rate_limit()
returns trigger language plpgsql security definer as $$
declare
  v_max_count    integer;
...
begin
  -- Only rate-limit ephemeral crowd reports. Everything else (open-data ingestion, durable/session
  -- crowd pins, block-scoped filming/construction which has its own limiter in 02f) is untouched.
  if new.source != 'crowd' or new.lifespan != 'ephemeral' then
    return new;
  end if;
  ...
  if v_recent_rows >= v_max_count then
    raise exception 'rate limit exceeded: max % ephemeral report(s) per % hour(s)', v_max_count, v_window_hours
...
create trigger pins_enforce_ephemeral_report_rate_limit
  before insert on public.pins
  for each row execute function public.enforce_ephemeral_report_rate_limit();
...
-- ============================================================
-- pg_cron/pg_net are already enabled and in production use (02d-ingest-cron.sql) — guarded
-- re-creation here is a defensive no-op if this migration is ever applied to a from-scratch project
-- before 02d.
